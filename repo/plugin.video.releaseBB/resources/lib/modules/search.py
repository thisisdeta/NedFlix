...
def reformat_date_in_title(title):
    ...
    def replace_match(match):
        ...
        day_num = int(day)
        return f" - {day_suffix(day_num)} {month_name} {year}"

    return re.sub(r'(\d{4})\.(\d{2})\.(\d{2})', replace_match, title)
    ...
    if 'new' == url:
        ...
        if keyboard.isConfirmed():
            ...
            try:
                ...
                for movieUrl, title, infos, domain in posts:
                    ...
                    name = '[COLORwhite]{0}[/COLOR]'.format(title)
        ...
        try:
            ...
            for movieUrl, title, infos, domain in posts:
                ...
                movieUrl = urljoin(base, movieUrl) if not movieUrl.startswith('http') else movieUrl
                title = six.ensure_str(title, 'utf-8')
                title = reformat_date_in_title(title)
                title = title.replace('AEW', 'AEW:')
                title = title.replace('Countdown', '- Countdown')
                title = title.replace('Buy In', '- The Buy In')                
                title = title.replace('TRILLERtV', '')
                title = title.replace('TrillerTV', '')
                title = title.replace('TRILLER', '')
                title = title.replace('MAX', '')
                title = title.replace('TBS', '')
                title = title.replace('TNT', '')
                title = title.replace('HDTV', '')
                title = title.replace('x264', '')
                title = title.replace('X264', '')
                title = title.replace('h264', '')
                title = title.replace('H264', '')
                title = title.replace('Webrip', '')
                title = title.replace('WEB', '')
                title = title.replace('Rip', '')
                title = title.replace('-DL', '')
                title = title.replace('-DJ', '')
                title = title.replace('-KYR', '')
                title = title.replace('-HEEL', '')
                title = title.replace('-NWCHD', '')
                title = title.replace('-Star', '')
                title = title.replace('-TJ', '')
